...
const OurExperts = ({ colorState = false }) => {
  ...
  useEffect(() => {
    ...
    const fetchEmployees = async () => {
      try {
        const res = await axios.get(`${API_URL}employees/employee-details/`);

        const formattedEmployees = res.data.data.map((emp, index) => ({
          id: emp.id || `emp-${index}`,
          image: emp.image || "https://placehold.co/600x800?text=Employee",
          name: emp.name,
          role: emp.role,
          description: emp.description || "Confidential!",
          address: emp.address || "Confidential!",
          phone: emp.phoneNumber || "Confidential!",
          facebook: emp.facebook || "https://www.facebook.com/dosmultiservices",
          instagram:
            emp.instagram || "https://www.facebook.com/dosmultiservices",
          linkedin: emp.linkedin || "https://www.facebook.com/dosmultiservices",
          twitter: emp.twitter || "https://www.facebook.com/dosmultiservices",
          portfolio:
            emp.portfolio || "https://www.facebook.com/dosmultiservices",
          portfolioState: !!emp.portfolio,
        }));
  ...
  return (
    <div>
      <div className="flex flex-col flex-1 items-center">
        <div className="max-w-8xl items-center mx-auto px-4 lg:items-center lg:w-full sm:px-6 lg:px-8 mt-12 lg:mt-10">
          <OurPeople colorState="F2F1FF" />
          <div className="max-w-8xl grid grid-cols-1 lg:-mt-20 sm:grid-cols-2 lg:w-full lg:items-center lg:justify-center xl:grid-cols-4 gap-6 lg:gap-6">
            {employees.map((person) => (
              <div
                key={person.id}
                className="group relative h-105 lg:items-center sm:h-115 lg:h-125 rounded-3xl overflow-hidden shadow-lg"
              >
                <img
                  src={person.image}
                  alt={person.name}
